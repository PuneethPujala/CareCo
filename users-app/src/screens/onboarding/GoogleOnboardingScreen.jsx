...
export default function GoogleOnboardingScreen({ navigation }) {
  ...
  const handleComplete = async () => {
    setSaveLoading(true);
    try {
      await completeGoogleSignUp("patient", city);
    } catch (error) {
      console.warn(error);
  ...
  const renderStep1 = () => (
    <View style={styles.stepContent}>
      ...
      <Text style={styles.stepDesc}>
        We'll assign a care caller available in your city.
      </Text>

      <Pressable
        style={[styles.citySelector, cityError && { borderColor: "#EF4444" }]}
        onPress={() => setCityModalVisible(true)}
      >
        <MapPin size={18} color="#94A3B8" />
        <Text style={[styles.citySelectorText, !city && { color: "#94A3B8" }]}>
          ...
          <View style={styles.modalSheet}>
            ...
            {["Google Pay", "PhonePe", "Paytm"].map((app) => (
              <Pressable
                ...
                accessibilityLabel={`Pay with ${app}`}
                accessibilityRole="button"
              >
            ...
            <Pressable
              style={styles.upiIdBtn}
              onPress={handlePaymentSuccess}
              accessibilityLabel="Pay with UPI ID"
              accessibilityRole="button"
            >
  ...
  const renderStep3 = () => (
    <View style={styles.stepContent}>
      ...
      <Text style={styles.stepDesc}>
        A Care Caller will call you within 24 hours to collect your health
        details, set up medications, and note your medical history.
      </Text>
      <View style={styles.callCard}>
        ...
        <Text style={styles.callItem}>
          📞 Assign your dedicated care caller
        </Text>
      </View>
      <Pressable
        style={styles.primaryBtn}
      ...
      >
        <Text style={styles.primaryBtnText}>
          {saveLoading ? "Creating Profile..." : "Go to Dashboard"}
        </Text>
      </Pressable>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.stepChip}>
          <Text style={styles.stepChipText}>{labels[step - 1]}</Text>
        </View>
      </LinearGradient>
...
const CityModal = ({ visible, onClose, onSelect, selectedCity }) => {
  ...
  const monthRef = React.useRef(null); // Added for new TextInput
  const yearRef = React.useRef(null); // Added for new TextInput

  const handleDigitInput = (text, setter, maxLength, nextRef) => {
    // Added for new TextInput
    setter(text);
    if (text.length === maxLength && nextRef && nextRef.current) {
          ...
          <View style={styles.searchWrap}>
            ...
            <View style={{ flex: 1, marginLeft: 10 }}>
              <TextInput
                ...
                placeholder="DD"
                value={""} // Placeholder for dobDay
                onChangeText={(t) => handleDigitInput(t, () => { }, 2, monthRef)} // Placeholder for setDobDay
                keyboardType="number-pad"
                maxLength={2}
                accessibilityLabel="Birth day input"
              />
            </View>
          </View>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                style={[
                  styles.cityRow,
                  item === selectedCity && styles.cityRowActive,
                ]}
                onPress={() => {
                  onSelect(item);
                  onClose();
              ...
              >
                <MapPin
                  size={16}
                  color={item === selectedCity ? "#3A86FF" : "#94A3B8"}
                />
                <Text
                  style={[
                    styles.cityText,
                ...
                {item === selectedCity && (
                  <CheckCircle2 size={18} color="#3A86FF" />
                )}
              </Pressable>
            )}
          />
...
const styles = StyleSheet.create({
  ...
  primaryBtn: {
    ...
    marginTop: 16,
    shadowColor: "rgba(58,134,255,0.35)",
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 1,
    shadowRadius: 12,
    elevation: 4,
