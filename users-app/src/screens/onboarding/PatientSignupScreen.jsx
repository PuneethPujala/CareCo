...
export default function PatientSignupScreen({ navigation, route }) {
    const { user, signUp, signInWithGoogle, completeSignUp, injectSession } = useAuth();
    ...
    useEffect(() => {
        ...
        // Fetch cities when reaching step 2
        if (step === 2 && availableCities.length === 0) {
            fetchCities();
        }
    }, [step]);
    ...
    const handleGoogleSignUp = async (idToken) => {
        try {
            ...
            if (result?.isNewUser) {
                // New Google user — register them in backend
                ...
                try {
                    ...
                    // Fetch the newly created profile using the session token directly
                    ...
                    // Inject the session and profile into global AuthContext so the user is authenticated
                    await injectSession(result.session, profileRes.data.profile);

                    // Proceed to next step
                    setStep(2);
                } catch (regError) {
                    setErrors({ google: regError?.response?.data?.error || regError.message || 'Failed to create account' });
                }
            }
            // If existing user, AuthContext auto-redirects to dashboard
        } catch (error) {
    ...
    const handleDetectLocation = async () => {
        ...
        try {
            ...
            const response = await apiService.patients.reverseGeocode(latitude, longitude);
            ...
            if (data?.address) {
                ...
                const addrStr = [city, state, postcode].filter(Boolean).join(', ');
            ...
            <Animated.View style={{ opacity: staggerAnims[2], width: '100%', alignItems: 'center' }}>
                ...
                >
                    ...
                    <Text style={styles.locationSecondaryBtnText}>
                        {loadingCities ? 'Loading cities...' : 'Select city manually'}
                    </Text>
                </Pressable>
                            ...
                            {loadingCities ? (
                                ...
                                filteredCities.map((city) => (
                                    <Pressable
                                        ...
                                        style={[
                                            styles.cityOption,
                                            form.city === city.name && styles.cityOptionActive
                                        ]}
                                        onPress={() => {
                                    ...
                                    >
                                        ...
                                        <View style={{ flex: 1, marginLeft: 16 }}>
                                            <Text style={[styles.cityName, form.city === city.name && { color: '#0EA5E9', fontWeight: '700' }]}>
                                                {city.name}
                                            </Text>
...
const styles = StyleSheet.create({
    ...
    cityOption: {
        ...
        backgroundColor: '#FAFBFC', borderWidth: 1, borderColor: '#E2E8F0',
